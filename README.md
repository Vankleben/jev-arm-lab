...
```
jev_arm/
  sim.py        MuJoCo 封装：IK（阻尼最小二乘）、伺服、夹爪、接触/真值、运动学携带
...
| 运行 | 结果 |
|---|---|
| `--jev-mode fake` | 6 个循环完成，落点水平误差 **1.9 mm** |
...
局限：都是单点故障、场景仍简单（单物体、平面桌、无人）；"抓取后滑脱"这类故障在当前
运动学携带的简化下**无法表达**——那恰恰是现实里最显眼的失败模式。
...
这个实验台的重点是**决策层**，物理上有两处简化，都在代码里标注了：
...
2. **搬运**：接触是真实的（夹爪真的合上、真的碰到方块，代码据此判定"抓到了"），但**握住之后
   的搬运用运动学携带**（`sim.py: _carry()`）—— 每步把物体贴回抓取时的相对位姿。
   原因是接触摩擦搬运在这个模型里没调通：30 N 夹持 + 摩擦系数 1.2 抓 50 g 方块，
   一提升就滑脱（法向力、摩擦、接触点都实测过，原因未查明）。要换成真实接触搬运，
   这是第一个该解决的技术债。
